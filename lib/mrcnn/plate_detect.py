...
class PlateDetect(metaclass=Singleton):
    ...
    def detect(self, src,model_dir):
        ...
        with self.graph.as_default():
            ...
            #加载模型和weights结束
            #打开摄像头
            print("Read Model Finshed!")
            result = self.model.detect([src])
            if len(result[0]['masks']) > 0 and len(result[0]['rois']) > 0:
            ...
            #     if len(result[0]['masks']) >0 and len(result[0]['rois']) > 0:
            #         plates = self._post_process(result, frame)
            #         if plates is not None:
            #             for plate in plates:
            #                 cv2.polylines(frame, np.int32([plate]), True, (0, 0, 255), 10)
            #                 cv2.putText(frame, "", (20, 100), font, 1, (0, 255, 0), 4)
            #
            #                 # print("Plate position: \n", plate)
            #                 # vis_image = align(frame, plate)
